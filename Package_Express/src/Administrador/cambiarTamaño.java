...
combo.removeAllItems();
 ...
 boolean validarTamaño;
public void Tamaño(){
    validarTamaño=true;
    int ruta=0;
for(int i=0; i<COLARUTA.size();i++){
   for(int j=0;j<COLARUTA.get(i).getA().size();j++){
       if(COLARUTA.get(i).getA().get(j).getId()==Integer.parseInt(combo.getSelectedItem().toString())){
           if(COLARUTA.get(i).getA().get(j).getCapacidad()-Integer.parseInt(tamaño.getText())<COLARUTA.get(i).getA().get(j).getDisponibilidad()){
           validarTamaño=true;
    ...
    boolean validar=true;
        if(validar()){
            ...
            JOptionPane.showMessageDialog(this,"Erro al modificar tamaño por datos en cola");
